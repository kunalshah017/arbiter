...
def test_binance_klines_to_bars():
    ...
    klines = [
        ...
        [1704070800000, "103.0", "107.0", "101.0", "106.0",
            "4000000.0", 1704074399999, "0", 0, "0", "0", "0"],
    ]
...
def test_bars_to_engine_json_format():
    bars = [OHLCVBar(ts=1700000000, open=100, high=105,
                     low=99, close=103, volume=1e6)]
    result = bars_to_engine_json(bars)
    assert result == [{"ts": 1700000000, "o": 100,
                       "h": 105, "l": 99, "c": 103, "v": 1e6}]
