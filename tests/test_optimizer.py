...
def make_trending_bars(n=200):
    ...
    for i in range(n):
        ...
        bars.append({"ts": 1700000000 + i * 3600, "o": price + noise, "h": price * 1.005, "l": price * 0.995, "c": price - noise, "v": 1e6})
    return bars
...
def test_validate_strategy_detailed():
    ...
    gate, raw = validate_strategy_detailed(json.dumps(bars), json.dumps(config))
    assert hasattr(gate, "passed")
    assert "trade_pnls" in raw


def test_optimization_result():
    r = OptimizationResult(status="accepted", strategy_config={"name": "test"}, iteration=1, total_iterations=3)
