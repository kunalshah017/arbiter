...
async def register_agent_identity():
    ...
    try:
        ...
        agent_uri = sdk.generate_agent_uri(
            ...
            endpoints=[
                AgentEndpoint(name="trading", endpoint="https://arbiter.agent", version="0.1.0"),
            ],
        )

        result = sdk.register_agent(agent_uri=agent_uri)
        logger.info("bnb_sdk.registered", agent_id=result.get("agentId"), tx=result.get("transactionHash"))
        return result

    except ImportError:
        logger.warning("bnb_sdk.not_installed", msg="bnbagent package not available")
        return None
